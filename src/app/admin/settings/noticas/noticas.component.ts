...
@Component({
  selector: 'app-admin-settings-noticas',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ],
  templateUrl: './noticas.component.html',
})
export class SettingNoticasComponent {
    ...
    getRecord(){
      ...
      this.service.find('noticias').subscribe({
        next: (value:any) => {
          this.data = value.data.noticias ? JSON.parse(value.data.noticias) : {};
