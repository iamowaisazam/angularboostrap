...
@Component({
  selector: 'app-admin-settings-escuela',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class SettingEscuelaComponent {
    ...
    getRecord(){
      ...
      this.service.find('escuela').subscribe({
        next: (value:any) => {
          this.data = value.data.escuela ? JSON.parse(value.data.escuela) : {};
