...
@Component({
  selector: 'app-admin-settings-repository',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class SettingRepositoryComponent {
    ...
    getRecord(){
      ...
      this.service.find('repository').subscribe({
        next: (value:any) => {
          this.data = value.data.repository ? JSON.parse(value.data.repository) : {};
