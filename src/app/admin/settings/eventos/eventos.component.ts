...
@Component({
  selector: 'app-admin-settings-eventos',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class SettingEventosComponent {
    ...
    getRecord(){
      ...
      this.service.find('eventos').subscribe({
        next: (value:any) => {
          this.data = value.data.eventos ? JSON.parse(value.data.eventos) : {};
